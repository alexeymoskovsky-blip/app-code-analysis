...
public class T3DeviceViewHolder extends BaseDeviceViewHolder {
    private Activity activity;

    public T3DeviceViewHolder(Activity activity, View view, IRecyclerItemClickListener iRecyclerItemClickListener) {
    ...
    public void updateData(DeviceCard deviceCard) {
        ...
        this.noticeFlag.setVisibility(DeviceCenterUtils.isT3NeedOtaById(deviceCard.getDeviceData().getData().getId()) ? 0 : 8);
        if (BleDeviceUtils.getT3RecordByDeviceId(deviceCard.getDeviceData().getData().getId()) != null || deviceCard.getDeviceData().getData().getDeviceShared() != null) {
            if (deviceCard.getDeviceData().getData().getDeviceShared() != null) {
                ...
                if (!TextUtils.isEmpty(name)) {
                    ...
                    this.name.setText(this.activity.getString(R.string.T3_name_default));
                }
            } else {
            ...
            if (deviceCard.getDeviceData().getData().getState() == 2) {
                this.icon.setImageResource(R.drawable.t3_home_item_icon);
        ...
        DeviceCenterUtils.setT3RecordUpdate(false);
